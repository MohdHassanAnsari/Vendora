...
export const ProductsDisplay: React.FC = () => {
  ...
  useEffect(() => {
    ...
    fetch("http://localhost:3000/products")
      ...
      .then((expressData: { result: ExpressDataItem[] }) => {
        // Fetch data from Django
        fetch("http://127.0.0.1:8000/restapi/users/")
          .then((response) => response.json())
          .then((djangoData: DjangoDataItem[]) => {
            // Combine data from both sources into a single array
            const combinedData: CombinedDataItem[] = [
              ...expressData.result.map((item: ExpressDataItem) => ({
                id: item._id,
                username: item.title ?? "No Title",
                email: item.description,
                picture: item.picture,
              })),
              ...djangoData.map((item: DjangoDataItem) => ({
                id: item.id.toString(),
                username: item.username,
                email: item.email,
                picture: "", // Add the default picture URL here if needed
              })),
            ];

            // Set the combined data in the component state
      ...
      .catch((error) => console.error("Error fetching Express data:", error));
  }, []);
